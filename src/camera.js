...
import { BG_COLOR, CAM_TRAIL_DISTANCE, CAM_TRAIL_HEIGHT, CAM_LERP, CAM_UP_LERP } from './config.js';
...
export function setupCamera(app, ship) {
    ...
    // Start already behind the ship so the first frame isn't a swoop-in. The
    // height offset and view up-vector follow the ship's OWN up, so a banked or
    // inverted ship (the free schemes) rolls the camera with it instead of
    // fighting a world-locked horizon.
    const start = new pc.Vec3()
        .copy(ship.forward).mulScalar(-CAM_TRAIL_DISTANCE)
        .add(ship.getPosition());
    start.add(new pc.Vec3().copy(ship.up).mulScalar(CAM_TRAIL_HEIGHT));
    camera.setPosition(start);
    camera.lookAt(ship.getPosition(), ship.up);

    app.root.addChild(camera);

    // Smoothed trailing direction and up-vector (world-space). Start aligned.
    const trailDir = new pc.Vec3().copy(ship.forward);
    const trailUp = new pc.Vec3().copy(ship.up);
    const camPos = new pc.Vec3();
    const upOffset = new pc.Vec3();

    function update(dt) {
        // Smooth only the direction and up we trail from. Frame-rate-independent
        ...
        const tu = 1 - Math.exp(-CAM_UP_LERP * dt);
        trailUp.lerp(trailUp, ship.up, tu);
        if (trailUp.lengthSq() > 1e-8) trailUp.normalize();

        // Rigid offset from the ship's real position -> constant distance. The
        // height is along the (smoothed) ship-up, not world-up, so the offset
        // direction rolls with the ship while its length stays fixed — the
        // anti-jitter property (constant distance) is preserved.
        camPos.copy(trailDir).mulScalar(-CAM_TRAIL_DISTANCE).add(ship.getPosition());
        camPos.add(upOffset.copy(trailUp).mulScalar(CAM_TRAIL_HEIGHT));
        camera.setPosition(camPos);
        camera.lookAt(ship.getPosition(), trailUp);
    }
