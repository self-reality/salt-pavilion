...
export function createSidebar(ctx) {
    ...
    slider(atm, 'Fog start', 0, 40, 0.5, 10, (v) => { scene.fog.start = v; });
    slider(atm, 'Fog end', 5, 120, 1, 55, (v) => { scene.fog.end = v; });
    slider(atm, 'Fog density', 0, 0.1, 0.001, 0.015, (v) => { scene.fog.density = v; });

    // ----- Lighting -----
    const lit = section('Lighting');
    let ambHex = '#8c8c8c', ambInt = 0.5;
    const applyAmbient = () => {
    ...
    slider(lit, 'Sun intensity', 0, 3, 0.01, 0.9, (v) => { light.light.intensity = v; });
    let yaw = 30, pitch = 50;
    ...
    slider(lit, 'Env intensity', 0, 3, 0.01, 1.0, (v) => { scene.skyboxIntensity = v; });
    ...
    slider(mat, 'Gloss', 0, 1, 0.01, 0.85, (v) => applyMat('gloss', v));
    slider(mat, 'Metalness', 0, 1, 0.01, 0.0, (v) => applyMat('metalness', v));
    slider(mat, 'Reflectivity', 0, 1, 0.01, 0.5, (v) => applyMat('reflectivity', v));

    // ----- Post -----
    const post = section('Post (halo)');
    select(post, 'Tone map', ['linear', 'neutral', 'aces', 'filmic'], 'neutral',
        (v) => { cf.rendering.toneMapping = TONE_MAPS[v]; cf.update(); });
    slider(post, 'Bloom intensity', 0, 0.2, 0.005, 0.02, (v) => { cf.bloom.intensity = v; cf.update(); });
    slider(post, 'Bloom blur', 1, 16, 1, 16, (v) => { cf.bloom.blurLevel = v; cf.update(); });
